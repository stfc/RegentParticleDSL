...
local rval = rquote
  regentlib.assert(first_invalid == part-1, "The particles in this cell are not contiguous which is unexpected.")
  end
  return rval
end
end 
    ...
    local rval = rquote
      ...
      for cell in neighbour_init.cell_partition.colors do
        for part in neighbour_init.cell_partition[cell] do
          regentlib.assert(neighbour_init.cell_partition[cell][part].neighbour_part_space.cell_id == int3d(cell), "Cell found in wrong cell")
        end
      end    
    end
    return rval
  ...
  for direction = 0, 26 do
    if(valid) then
      ...
      for cell in [neighbour_init.cell_partition].colors do
        ...
        valid = valid and tradequeue_push([neighbour_init.cell_partition][cell], neighbour_init.TradeQueues[tradequeueid], cell, neighbour)
      end
      ...
      for cell in [neighbour_init.cell_partition].colors do
        ...
        var neighbour : int3d = int3d({new_x, new_y, new_z})
        var tradequeueid = cell.x + cell.y * [variables.config][0].neighbour_config.x_cells + cell.z * [variables.config][0].neighbour_config.x_cells * [variables.config][0].neighbour_config.y_cells
        valid = valid and tradequeue_pull([neighbour_init.cell_partition][neighbour], neighbour_init.TradeQueues[tradequeueid])
      end
    end
  end
  ...
  --Find end of array
    ...
    for part = lo, hi+1 do
      if [neighbour_init.padded_particle_array][int1d(part)].neighbour_part_space._valid then
        [contig_assertion(start_index, part)];
        start_index = part + 1
      end
    end
