...
--In 2D we only need 8 tradequeues
...
local __demand(__leaf) task tradequeue_push( cell_id : int2d,
...
do
                      ...
                      __quotes:insert(rquote
                        if particles[part].neighbour_part_space._transfer_dir == 0 and
                           (particles[part].neighbour_part_space.cell_id
                            == (cell_id + [directions[i]] + {count_xcells,count_ycells})%{count_xcells,count_ycells} )
                           then
    ...
    if toTransfer ~= 0 then
        ...
        format.snprintln(s,512, "Particle moving to non-adjacent cell from cell {} {}", cell_id.x, cell_id.y)
        regentlib.assert(toTransfer == 0, s)
        regentlib.c.free(s)
    end

   --Loop through the queues, clear them and copy the particle data. Each cell owns its own section of the queue for each direction so                                                         --don't need to worry about any other cell touching it.
    [(function() local __quotes = terralib.newlist()
        for i=1, 8 do
               ...
               --Count how many to transfer and where they belong
                ...
                if int1d(transferred) > queue.bounds.hi - queue.bounds.lo + 1 then
                    ...
                    format.snprintln(s, 512, "Transferring more particles than fit in the tradequeue. Cell {} {}, transfers: {}",
                                     cell_id.x, cell_id.y, int32(transferred))
...
local __demand(__leaf) task tradequeue_pull( cell_id : int2d,
                                             particles : region(ispace(int1d), part),
                                             config : region(ispace(int1d), config_type),
                                             [tradequeues] ) where
      reads(config), reads writes(particles),
      --We read from all the fields in each of the tradeQueue partitions
      [tradequeues:map(function(queue)
        return part_structure:map(function (element)
...
do
    ...
    if available_slots < total_transfers then
        var s : rawstring
        s = [rawstring] (regentlib.c.malloc(512))
        format.snprintln(s,512, "Not enough space in cell {} {} to accept incoming particles. {} space and {} required", cell_id.x, cell_id.y, 
...
local __demand(__inline) task partition_tradequeue_by_cells( tradequeue : region(ispace(int1d), part),
    ...
    for cell in cell_space do
      ...
      var color : int2d = (cell - offset + {count_xcells, count_ycells}) % {count_xcells, count_ycells}
      --Indices for this color are number of elements per cell (count/n_cells) * (3D to 1D conversion)
      var oneD_color : int1d = (color.x*count_ycells) + (color.y);
      var rect = rect1d{
        ...
        for part in neighbour_init.cell_partition[cell] do
          if neighbour_init.cell_partition[cell][part].neighbour_part_space._valid then
            if(neighbour_init.cell_partition[cell][part].neighbour_part_space.cell_id ~= int2d(cell)) then
                format.println("Particle {} expected in {} {} and found in {} {}", neighbour_init.cell_partition[cell][part].core_part_space.id,
...
local update_cells_quote = rquote
    ...
    for cell in [neighbour_init.cell_partition].colors do
        tradequeue_push(cell, [neighbour_init.cell_partition][cell], [variables.config],
                        [generate_range_as_terralist(1, 8):map(function(i) return rexpr
...
--Init Tradequeues                                                                                                                                                           --The wrapper here is an inline way to create this for all 26 directions inside Regent code, since we're using
