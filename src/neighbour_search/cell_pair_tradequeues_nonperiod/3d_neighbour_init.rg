...
local __demand(__leaf) task compute_new_dests(particles : region(ispace(int1d), part), config : region(ispace(int1d), config_type)) where
  ...
  for particle in particles do
    ...
    if z_cell >= int1d(config[0].neighbour_config.z_cells) then
       z_cell = config[0].neighbour_config.z_cells-1
    end
    if z_cell < int1d(0) then
       z_cell = 0
    end
      var cell_loc : int3d = int3d( {x_cell, y_cell, z_cell} )                                                                                                                   particles[particle].neighbour_part_space.cell_id = cell_loc
    end
  end
end
