  ...
  for direction = 0, 26 do
    if(valid) then
      ...
      for cell in [neighbour_init.cell_partition].colors do
        ...
        if (new_x >= 0 and new_x < [variables.config][0].neighbour_config.x_cells)
           and (new_y >= 0 and new_y < [variables.config][0].neighbour_config.y_cells)
           and (new_z >= 0 and new_z < [variables.config][0].neighbour_config.z_cells) then
