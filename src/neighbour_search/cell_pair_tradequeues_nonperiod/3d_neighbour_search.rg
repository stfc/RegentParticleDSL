...
local __demand(__leaf) task pairwise_task([parts1], [parts2], config : region(ispace(int1d), config_type))
  ...
  reads(config), reads(parts1.core_part_space.{pos_x, pos_y, pos_z, cutoff}),
...
function create_asymmetric_pairwise_runner_multikernel( config, cell_space, ... )
  ...
  --Create the tasks
  local cell_pair_task, update_neighbours1 = generate_asymmetric_pairwise_task_multikernel( kernels, read1, read2, write1, reduc1 )
  local cell_self_task, update_neighbours2 = generate_asymmetric_self_task_multikernel( kernels, read1, read2, write1, reduc1 )
  local update_neighbours = update_neighbours1 or update_neighbours2
  local update_neighbours_quote = rquote
