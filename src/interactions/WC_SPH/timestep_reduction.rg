...
local timestep_quote = rquote
  ...
  if part.is_wall == WALL then
    ...
    part.a_hydro_x += part.a_const_x
    part.a_hydro_y += part.a_const_y
    part.a_hydro_z += part.a_const_z
  --Ignoring shifting for now
  if(part.since_euler == 0) then
    var temp : float = part.rho
    part.rho_t_minus1 = temp
    part.rho += part.drho_dt*config.space.timestep
    part.core_part_space.pos_x += part.core_part_space.vel_x*config.space.timestep + 0.5*part.a_hydro_x*config.space.timestep*config.space.timestep
    part.core_part_space.pos_y += part.core_part_space.vel_y*config.space.timestep + 0.5*part.a_hydro_y*config.space.timestep*config.space.timestep
    part.core_part_space.pos_z += part.core_part_space.vel_z*config.space.timestep + 0.5*part.a_hydro_z*config.space.timestep*config.space.timestep

    temp = part.core_part_space.vel_x
    part.core_part_space.vel_x += part.a_hydro_x*config.space.timestep
    part.v_minus1_x = temp
    
    temp = part.core_part_space.vel_y
    part.core_part_space.vel_y += part.a_hydro_y*config.space.timestep
    part.v_minus1_y = temp

    temp = part.core_part_space.vel_z
    part.core_part_space.vel_z += part.a_hydro_z*config.space.timestep
